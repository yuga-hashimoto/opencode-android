...
ローカルOpenCodeで利用できるモデルは、OpenCodeランタイムが返すプロバイダー一覧から自動的に表示される。Androidアプリ内でモデル本体を追加するのではなく、プロバイダーを認証してから一覧を更新する。

### APIキーを使う場合

1. 設定画面の「プロバイダーAPIキー」でProvider ID（例: `openai`）を入力する。
2. APIキーを入力して保存する。
3. ローカルランタイムを再起動、またはプロバイダー一覧を更新する。
4. チャット画面下部のモデル選択からモデルを選ぶ。

APIキーはAndroid Keystore対応の暗号化設定へ保存され、ローカルOpenCode起動時にランタイム内の `auth.json` へ同期される。

### ChatGPT Plus/Proを使う場合

OpenCodeがOpenAI OAuthを提供している場合は、設定画面のOpenAI認証から「ChatGPT Plus/Pro」を選ぶ。Androidのブラウザでログインし、OpenCodeが案内する認証を完了してからアプリへ戻る。認証情報はOpenCode側の `auth.json` が管理し、AndroidアプリはAPIキーとして保存しない。

OAuth方式が表示されない場合は、現在のOpenCodeランタイムがOAuthエンドポイントに対応していないため、OpenAI APIキー方式を使用する。OpenAIのChatGPTサブスクリプションとAPI利用料金は別契約である点に注意する。

### モデル一覧を更新する場合

OpenCodeのプロバイダー認証後に、チャット画面または設定画面を開き直すと一覧を再取得する。OpenCode設定ファイルだけに定義された任意のカスタムモデルをAndroidから編集する機能は、現在のAPKには含まれない。
