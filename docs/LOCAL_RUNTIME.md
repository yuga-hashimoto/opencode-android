...
- 設定画面からのプロバイダーAPIキー登録と、OpenCode OAuth対応プロバイダーの認証開始
...
利用可能な認証方法は、選択中のOpenCodeランタイムの `GET /provider/auth` から取得する。Android側でOpenAIやAnthropicなどを固定定義せず、ランタイムが返したすべてのプロバイダー、APIキー方式、OAuth方式、追加質問項目をそのまま表示する。

### プロバイダーへ接続する

1. 先にローカルまたはリモートのOpenCode実行先を選択する。
2. 設定画面の「プロバイダー接続」で対象プロバイダーを選ぶ。
3. OpenCodeが提示した認証方法を選ぶ。
4. 必要に応じてリージョン、アカウント、テナントなどの追加項目を入力する。
5. APIキー方式ではキーを入力する。OAuth方式ではブラウザの案内に従う。

APIキーは `PUT /auth/{providerID}`、切断は `DELETE /auth/{providerID}` を使い、選択中のOpenCode実行先へ直接反映する。このため、Androidローカルだけでなく、Mac・Linux・Windows上のリモートOpenCodeにも同じ画面から設定できる。

### OAuth方式

OpenCodeが `auto` 方式を返した場合、Androidはブラウザを開くと同時にOAuth完了を待機する。ユーザーが手動で「完了」を押す必要はない。OpenCodeが確認コードを案内する場合は、ダイアログ内に選択・コピー可能なコードを表示する。

OpenCodeが `code` 方式を返した場合は、ブラウザで取得した認証コードをAndroidへ入力して完了する。OAuthトークンはAndroidアプリではなく、接続先OpenCodeの `auth.json` が管理する。

### ローカルAPIキーの互換性

旧バージョンでAndroid Keystore対応設定に保存したAPIキーは、ローカルランタイム起動前のブートストラップ用途として引き続き利用できる。新しい設定画面から認証したプロバイダーはOpenCode側が所有し、OAuth情報や新しいAPIキーを古い同期処理で上書きしない。

### モデル一覧

認証または切断が成功すると、プロバイダーとモデルのカタログを自動的に再取得する。OpenCode設定ファイルだけに定義された任意のカスタムモデルをAndroidから編集する機能は含まれない。
